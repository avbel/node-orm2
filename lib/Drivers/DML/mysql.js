...
Driver.prototype.find = function (fields, table, conditions, opts, cb) {
    ...
    this.execQuery(q, function(err, items){
        if(items){
            var processDates = function(item){
                for(var k in item){
                    var v = item[k];
                    if(v instanceof Date){
                        d = new  Date();
                        d.setUTCFullYear(v.getFullYear()); 
                        d.setUTCMonth(v.getMonth());       
                        d.setUTCDate(v.getDate());       
                        d.setUTCHours(v.getHours());       
                        d.setUTCMinutes(v.getMinutes());       
                        d.setUTCSeconds(v.getSeconds());       
                        d.setUTCMilliseconds(v.getMilliseconds());
                        item[k] = d;
                    }
                }
            };
